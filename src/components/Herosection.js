...
useEffect(() => {
  const getdata = async () => {
    try {
      const response = await axios.get("https://next-pos-backend.vercel.app/api/get/transactiongraph");
  ...
  useEffect(() => {
    ...
    const latesttransaction = async () => {
      try {
        const responsetransaction = await axios.get(
          `https://next-pos-backend.vercel.app/api/get/transactions`,
    ...
    const getBlockInfo = async () => {
      const responseblocks = await axios.get(
        `https://next-pos-backend.vercel.app/api/get/blocks`,
        { withCredentials: true }
      );
